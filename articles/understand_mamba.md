...
### Q. Attentionに相当する工夫は、Mambaにあるのか

**A.** Attention行列（$\mathrm{softmax}(QK^T / \sqrt{d})$）に相当する"全対全"の重み行列は、Mambaには存在しない。

その代わり、Mambaは```選択的SSM```として、各トークンごとに生成される入力依存の状態更新と読み出し（$\bar{A}_t, \bar{B}_t, C_t, \Delta_t$）で暗黙の「重み」を作り、```再起(scan)```で系列を混合する。…この暗黙の重みは動的畳み込み核（下三角Toeplitz行列）として解釈でき、attention類似の"誰が誰に効くか"をように行列化せずに実現する。…？